...
def report(logs, target_field, output, avg):
...
def plot_pandas(logs, target_field, columns):
    columns = logs.keys() if columns is None else columns
    columns = list(set(columns) - set([target_field, "epoch"]))
    ret = logs.pivot_table(index="epoch", columns=columns, values=target_field)
    ret.plot()
    pylab.gca().get_legend().draggable()
    pylab.show()

def plot_confidence(logs, target_field, columns):
    columns = logs.keys() if columns is None else columns
    columns = list(set(columns) - set([target_field, "epoch"]))
   ...
   # logs = pd.DataFrame(logs, index=logs[target_field].unstack().dropna(how='all').stack().index)
    ret = logs.pivot_table(index="epoch", columns=columns, values=target_field, aggfunc=[np.nanmin, np.nanmax, np.nanmean])
...
def main(args):
    ...
    if args.command == "s" or args.command == "summarize":
        report(logs, args.target_field, args.summary_output, args.avg)
    elif args.command == "p" or args.command == "plot":
        if args.confidence:
            plot_confidence(logs, args.target_field, columns)
        else:
            plot_pandas(logs, args.target_field, columns)
